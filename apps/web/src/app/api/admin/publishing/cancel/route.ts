...
export async function POST(request: Request) {
  ...
  try {
    ...
    const pub = (await client.query<{ id: string; status: string; scheduled_for: string | null }>(
      `SELECT id,status,scheduled_for FROM scheduled_publications WHERE id=$1 FOR UPDATE`,
    ...
    await client.query(`UPDATE scheduled_publications SET status='cancelled',error=$2 WHERE id=$1`, [parsed.data.publicationId, parsed.data.reason])
