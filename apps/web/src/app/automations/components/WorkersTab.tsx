...
import { Button, DataGrid, StatusBadge, createColumnHelper, gridFeatures } from '@plataforma/ui-bridge'
...
export function WorkersTab({ workers: initialWorkers }: { workers: WorkerInfo[] }) {
  ...
  return (
    <div style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {groups.map((group) => (
        <section key={group.domain} className="bridge-section">
          <h2>{labels[group.domain] ?? group.domain}</h2>
          <DataGrid data={group.workers} columns={columns} enableSorting label={`Workers de ${labels[group.domain] ?? group.domain}`} />
        </section>
      ))}
    </div>
  )
