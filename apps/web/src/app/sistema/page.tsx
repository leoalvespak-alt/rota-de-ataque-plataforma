...
export default async function SystemPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <ModulePage eyebrow="Prontidão e administração" title="Sistema" subtitle="Estado desejado, runtime, prontidão, incidentes e integrações sem poluir o caminho operacional." metrics={[{ label: 'Motores saudáveis', value: healthyEngines, unit: `/ ${AUTOMATION_ENGINES.length}`, period: 'estado atual', sourceStatus: 'ready', href: '/sistema/motores' }, { label: 'Bloqueios de ativação', value: workers.filter((worker) => !worker.enabled && worker.heartbeat_state !== 'running').length, period: 'requisitos pendentes', sourceStatus: 'ready', href: '/sistema/motores' }, { label: 'Incidentes abertos', value: openIncidents, period: 'não resolvidos', sourceStatus: openIncidents ? 'degraded' : 'ready', href: '/sistema/incidentes' }, { label: 'Integrações prontas', value: capabilities.filter((capability) => capability.status === 'ready').length, period: 'checklist atual', sourceStatus: 'ready', href: '/sistema/integracoes' }]} navigation={[{ label: 'Visão geral', href: '/sistema' }, { label: 'Motores', href: '/sistema/motores' }, { label: 'Integrações', href: '/sistema/integracoes' }, { label: 'Incidentes', href: '/sistema/incidentes' }, { label: 'Avançado', href: '/sistema/avancado/workers' }]} main={<><header className="module-panel-header"><div><p className="module-eyebrow">Checklist de prontidão</p><h2>O que está pronto para operar</h2></div><Link href="/sistema/integracoes">Configurar integração →</Link></header><div className="module-list">{capabilities.map((capability) => <article key={capability.id}><span className={`source-status ${capability.status === 'ready' ? 'ready' : capability.status === 'degraded' || capability.status === 'partial' ? 'degraded' : capability.status === 'disabled' ? '' : 'blocked'}`}>{capability.status}</span><strong>{capability.name}</strong><span>{capability.detail}</span>{capability.missing.length > 0 && <small aria-label="Pré-requisitos ausentes">Pré-requisitos ausentes: {capability.missing.join(', ')}</small>}</article>)}</div></>} rail={<><section><p className="module-eyebrow">Reddit</p><h2>{externalReddit?.status === 'ready' ? 'Via provedores externos' : 'Canário pendente'}</h2><p>{externalReddit?.detail ?? 'Configure Apify ou Bright Data para habilitar a coleta.'}</p><Link href="/sistema/integracoes">Ver prontidão →</Link></section><section><p className="module-eyebrow">Avançado</p><h2>Divulgação progressiva</h2><p><Link href="/sistema/avancado/workers">Workers</Link> · <Link href="/sistema/avancado/filas">Filas</Link> · <Link href="/sistema/avancado/agendamentos">Agendamentos</Link></p><p><Link href="/sistema/avancado/runbooks">Runbooks</Link> · <Link href="/sistema/avancado/ia">IA</Link> · <Link href="/sistema/avancado/scoring">Scoring</Link></p></section></>} />
}
