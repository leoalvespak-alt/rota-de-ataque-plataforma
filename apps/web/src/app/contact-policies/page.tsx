...
pool.query(`SELECT lead.id,lead.username_current FROM leads lead JOIN lead_scores score ON score.lead_id=lead.id WHERE ($1::uuid IS NULL OR score.campaign_id=$1) ORDER BY score.final_score DESC LIMIT 100`,[selected?.id??null])]);return <ContactPoliciesClient initialPolicies={policies.rows} campaigns={campaigns} selectedCampaignId={selected?.id??''} leads={leads.rows}/>}finally{}}
