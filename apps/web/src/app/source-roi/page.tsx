...
export default async function SourceRoiPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  try {
    ...
    const [result] = await Promise.all([pool.query<SourceRoiRow>(query, values), pool.query<{ total: number }>(countQuery, values.slice(0, 3))])
    return <SourceRoiClient data={result.rows.slice(0, DATA_PAGE_SIZE)} page={params.page} hasNext={result.rows.length > DATA_PAGE_SIZE} from={params.from} to={params.to} campaignName={selected?.name ?? null} />
  } finally {}
}
