...
interface ContentSuggestion { id: string; source_type: string; title: string; description: string | null; suggested_format: string | null; suggested_channel: string | null; pillar: string | null; evidence: Record<string, unknown>; curation_status: string; created_at: string }
...
export function ReviewInboxClient({ initialItems, decidedToday, radarFindings = [], competitorInsights = [], contentSuggestions = [] }: { initialItems: ReviewItem[]; decidedToday: number; radarFindings?: RadarFinding[]; competitorInsights?: CompetitorInsight[]; contentSuggestions?: ContentSuggestion[] }) {
  ...
  const openSuggestionDialog = useCallback((kind: 'edit-suggestion' | 'reject-suggestion', suggestion: ContentSuggestion) => {
    setSelectedSuggestion(suggestion); setSuggestionTitle(suggestion.title); setDialogReason(''); setDialogError(''); setDialog(kind)
  }, [])
  ...
  const submitDialog = useCallback(async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (dialog === 'slot' && selectedFinding) {
        ...
        ok = await postOrganicAction(`/api/admin/content-suggestions/${selectedSuggestion.id}/action`, { action: 'edit-approve', title: suggestionTitle.trim() }, () => setSuggestions((rows) => rows.filter((row) => row.id !== selectedSuggestion.id)))
  ...
  }, [dialog, dialogBusy, dialogReason, postOrganicAction, selectedFinding, selectedSuggestion, slotCaption, slotChannel, slotScheduledFor, slotTitle, suggestionTitle])
  ...
  return <div className="page" style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
    ...
    <Dialog open={dialog !== null} onOpenChange={(open) => { if (!open && !dialogBusy) setDialog(null) }} title={dialog === 'slot' ? 'Criar slot editorial' : dialog === 'dismiss-radar' ? 'Descartar achado do radar' : dialog === 'edit-suggestion' ? 'Editar e aprovar sugestão' : 'Rejeitar sugestão'}>
      <form onSubmit={submitDialog} style={{ display: 'grid', gap: 'var(--space-3)' }}>
        ...
        {dialog === 'edit-suggestion' && <InputField label="Título" value={suggestionTitle} onChange={(event) => setSuggestionTitle(event.target.value)} maxLength={180} autoFocus />}
