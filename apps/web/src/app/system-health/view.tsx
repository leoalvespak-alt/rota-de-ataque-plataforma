  ...
  try {
    ...
    const [heartbeats, alerts, health, canaries, capabilities, killSwitch, workerSettings] =
      await Promise.all([
        ...
        pool.query(
          `SELECT id,reason_code kind,CASE WHEN retryable THEN 'warn' ELSE 'error' END severity,occurred_at created_at FROM automation_incidents WHERE resolved_at IS NULL ORDER BY occurred_at DESC LIMIT 50`,
        ),
    ...
    const queueCounts = QUEUE_NAMES.map((worker) => ({
      worker,
      desired: enabledByDb.get(worker) ?? false,
      waiting: 0,
      delayed: 0,
      active: 0,
      failed: 0,
