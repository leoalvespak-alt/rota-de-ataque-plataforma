...
describe('navegação simplificada', () => {
  it('declara sete destinos principais', () => {
    ...
    expect(NAVIGATION.filter((destination) => destination.tier === 'simple')).toHaveLength(7)
  ...
  it('preserva os 28 pontos de entrada anteriores: raiz mais 27 redirects permanentes', async () => {
    ...
    for (const [legacyPath, target] of Object.entries(LEGACY_REDIRECTS)) {
      ...
      const [canonicalPath, query = ''] = target.split('?')
      const tab = new URLSearchParams(query).get('aba')
      ...
      expect(redirectSource).toContain(`'${tab}'`)
  ...
  it('mantém detalhe de conteúdo e contratos acessíveis das abas', async () => {
    ...
    const mode = await readFile(path.join(sourceDirectory, 'components', 'UiModeProvider.tsx'), 'utf8')
    expect(tabs).toMatch(/role="tablist"/u)
    expect(tabs).toMatch(/ArrowRight/u)
    expect(tabs).toMatch(/ArrowLeft/u)
    expect(mode).toMatch(/prospector_ui_mode/u)
    expect(mode).toMatch(/revealAdvanced/u)
  })
})
