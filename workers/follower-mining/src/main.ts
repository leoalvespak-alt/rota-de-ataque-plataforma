      ...
      for (const item of added) {
        ...
        await client.query(
          ...
          [
            leadId,
            snapshot.rows[0]!.id,
            JSON.stringify({ competitorId: target.competitorId, traceId }),
          ],
        );
      }
      await client.query(
...
const collector = {
  collect: (target: { username: string }, accountId: string) =>
...
const store = {
  async put(key: string, data: Buffer) {
    ...
    await s3.putObject(s3Bucket, key, compressed, compressed.length, {
...
const worker = runWorker(
  spec.queue,
  createFollowerProcessor(repository, collector, store, queue),
);
