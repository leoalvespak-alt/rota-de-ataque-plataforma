...
} from "@plataforma/notifications";
import {
  createQueueRegistry,
} from "@plataforma/queue";
import { runWorker } from "@plataforma/queue/runtime";
import { QUEUE_NAMES, type ErrorEvent, type QueueName } from "@plataforma/shared";
...
const configuredChannels = (): Record<
  string,
...
> => ({
  email:
    ...
    (process.env.ALERT_EMAIL_TO ?? process.env.ALERTS_EMAIL_TO)
...
async function routeEvent(event: ErrorEvent, routeTraceId: string) {
  ...
  const eventFingerprint = fingerprint(
    event.source,
    event.metric ?? event.worker,
    `${event.reason_code}:${event.error}`,
  );
  const existing = (
    await pool.query<{
      id: string;
  ...
  const alert = existing
    ...
    : (
       ...
       VALUES($1, $2, $3::jsonb, $4) RETURNING id`,
          [
            event.metric ?? event.reason_code,
            severity,
            JSON.stringify({
              ...event,
              occurrences,
  ...
  if (trigger) {
    await pool.query(
      "UPDATE error_triggers SET hit_count = hit_count + 1, last_hit_at = now() WHERE id = $1",
      [trigger.id],
    );
  }
...
const repository: AlertRepository = {
  async checkDeadMan(expectedIntervalSeconds, stagnantWindows, traceId) {
    ...
    let opened = 0;
    let resolved = 0;
    // Use PostgreSQL as the canonical source; env vars are only a fallback when no DB row exists
    const workerSettings = await pool.query<{ worker_name: string; enabled: boolean }>(
      `SELECT worker_name, enabled FROM worker_settings`,
    );
    const enabledByDb = new Map(workerSettings.rows.map((r) => [r.worker_name, r.enabled]));
    const enabledWorkers = QUEUE_NAMES.filter((worker) => {
      if (enabledByDb.has(worker)) return enabledByDb.get(worker);
      return process.env[`WORKER_${worker.replaceAll("-", "_").toUpperCase()}_ENABLED`] === "true";
    });
    for (const worker of enabledWorkers) {
      const present = heartbeats.rows.some(
        (heartbeat) =>
          heartbeat.worker === worker &&
          heartbeat.age_seconds <= expectedIntervalSeconds * 3,
      );
      if (!present) {
        await routeEvent(
          {
            ...
            severity: "critical",
            reason_code: "TIMEOUT",
            error: "Enabled worker has no current heartbeat",
            metric: "heartbeat_presence",
            observed: 0,
            threshold: 1,
            ...
            context: { worker_instance: `${worker}:missing` },
          },
          traceId,
        );
        opened++;
    ...
    for (const heartbeat of heartbeats.rows) {
      // Do not raise dead-man alerts for intentionally disabled workers
      if (enabledByDb.has(heartbeat.worker) && !enabledByDb.get(heartbeat.worker)) continue;
      const key = `${heartbeat.worker}:${heartbeat.instance_id}`;
      const isStale = heartbeat.age_seconds > expectedIntervalSeconds * 3;
      const isStagnant =
        heartbeat.backlog_seen > 0 && heartbeat.jobs_done_window === 0;
      const prior = (
      ...
      const windows = isStagnant
        ? Number(prior?.payload?.stagnant_windows ?? 0) + 1
        : 0;
        ...
        await routeEvent(
          {
            ...
            reason_code: "TIMEOUT",
            error: isStale
              ? "Worker heartbeat exceeded three expected intervals"
              : "Worker backlog is stagnant",
  ...
  async runCanaries(pipelines, timeoutMs, traceId) {
    let passed = 0;
    let failed = 0;
    const enabledPipelines = pipelines.filter(
      (pipeline) =>
        process.env[
          `WORKER_${pipeline.replaceAll("-", "_").toUpperCase()}_ENABLED`
        ] === "true",
    );
        ...
        await routeEvent(
          {
            ...
            reason_code: "TIMEOUT",
            error:
              "Synthetic canary did not complete within the latency objective",
