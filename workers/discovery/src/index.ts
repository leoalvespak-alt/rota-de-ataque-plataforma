...
export const discoveryPayloadSchema = z.discriminatedUnion('mode', [
  ...
  z.object({ mode: z.literal('social_collect'), campaignId: z.string().uuid(), platform: z.enum(['instagram', 'tiktok', 'youtube', 'reddit']), urls: z.array(z.string().url()).min(1).max(25), limit: z.number().int().min(1).max(500).optional(), watchId: z.string().uuid().optional() }).strict(),
  z.object({ mode: z.literal('fallback_collect'), campaignId: z.string().uuid(), platform: z.enum(['web', 'x', 'google', 'reddit']), urls: z.array(z.string().url()).min(1).max(10), limit: z.number().int().min(1).max(10).optional(), fallbackReason: z.enum(['primary_not_supported', 'primary_failed', 'primary_incomplete', 'validation_sample']), watchId: z.string().uuid().optional() }).strict(),
])
...
export interface DiscoveryRepository {
  ...
  complete(runId: string, reservationId: string, plan: ProviderPlan, payload: DiscoveryPayload, result: DiscoveryResult, traceId: string): Promise<{ inserted: number; candidates: number }>
  fail(runId: string, reservationId: string, error: unknown, payload?: DiscoveryPayload): Promise<void>
}
...
export function createDiscoveryProcessor(repository: DiscoveryRepository, provider: DiscoveryProvider) {
  ...
  return async (job: WorkerJob<DiscoveryPayload>): Promise<WorkerResult> => {
    ...
    try {
      ...
      const saved = await repository.complete(run.runId, run.reservationId, plan, parsed.data, result, base.traceId)
    ...
    } catch (error) { await repository.fail(run.runId, run.reservationId, error, parsed.data); throw error }
  }
}
