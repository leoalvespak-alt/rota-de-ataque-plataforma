...
export interface ThreadsAdapterRepository { get(id: string): Promise<ThreadsItem | null>; recentTexts(campaignId: string): Promise<string[]>; save(itemId: string, text: string): Promise<string>; createReview(variantId: string, traceId: string): Promise<void> }
...
export function createThreadsAdapter(repository: ThreadsAdapterRepository, generate: ThreadsGenerator) {
  const gate = createWorker<ThreadsAdapterPayload>(spec)
  return async (job: WorkerJob<ThreadsAdapterPayload>): Promise<WorkerResult> => {
    const base = await gate(job); const item = await repository.get(job.payload.contentItemId)
    if (!item) throw Object.assign(new Error('Content item not found'), { reasonCode: 'PREFLIGHT_FAILED' })
    const text = (await generate(threadsPrompt(item), await repository.recentTexts(item.campaignId))).trim()
    parseContentVariantPayload('threads', 'text', { text })
    const variantId = await repository.save(item.id, text); await repository.createReview(variantId, base.traceId)
    return { ...base, event: { kind: 'threads.variant_ready', payload: { contentItemId: item.id, variantId } } }
  }
}
