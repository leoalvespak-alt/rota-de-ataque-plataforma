...
if (!databaseUrl || !redisUrl || !accessToken || !igUserId) throw new Error('DATABASE_URL, REDIS_URL, META_ACCESS_TOKEN and META_INSTAGRAM_ACCOUNT_ID are required')

const { pool } = createDatabase(databaseUrl)
const registry = createQueueRegistry(redisUrl)
const api = new MetaApiClient(accessToken, process.env.META_API_VERSION ?? 'v21.0')

const repository: MetaSyncRepository = {
  async activeCompetitors(filter) {
    const values: string[] = []
    const clauses = ["cc.status='active'", "c.status='active'"]
    if (filter?.campaignId) { values.push(filter.campaignId); clauses.push(`cc.campaign_id=$${values.length}`) }
    if (filter?.competitorId) { values.push(filter.competitorId); clauses.push(`c.id=$${values.length}`) }
    const result = await pool.query<CompetitorTarget>(`SELECT cc.campaign_id AS "campaignId",c.id AS "competitorId",c.username FROM campaign_competitors cc JOIN competitors c ON c.id=cc.competitor_id WHERE ${clauses.join(' AND ')}`, values)
    return result.rows
  },
  async updateCompetitor(target, profile) {
    await pool.query(`UPDATE competitors SET instagram_user_id=COALESCE($2,instagram_user_id),followers_count=COALESCE($3,followers_count),follows_count=COALESCE($4,follows_count),media_count=COALESCE($5,media_count),biography=COALESCE($6,biography),profile_picture_url=COALESCE($7,profile_picture_url),last_synced_via_api_at=now() WHERE id=$1`, [target.competitorId, profile.id, profile.followers_count, profile.follows_count, profile.media_count, profile.biography, profile.profile_picture_url])
  },
  async upsertPost(target, media) {
    const shortcode = media.id!
    const result = await pool.query<{ id: string; inserted: boolean }>(`INSERT INTO posts(competitor_id,shortcode,post_url,posted_at,comment_count_shown,like_count_shown,caption,engagement_metrics,source)
      ...
      RETURNING id,(xmax=0) AS inserted`, [target.competitorId, shortcode, media.permalink, media.timestamp ?? null, media.comments_count ?? 0, media.like_count ?? 0, media.caption ?? null, JSON.stringify({ media_type: media.media_type })])
    return result.rows[0]!
  },
  async saveOwnSnapshot(accountId, profile, media, insights, mentions, conversations) {
  ...
  async incrementRateLimit(accountId) {
    await pool.query(`INSERT INTO account_health(account_id,api_rate_limited_count) VALUES($1,1)`, [accountId])
  },
}

const queue = {
  extraction: (postId: string, runId: string, payload: Record<string, unknown>) => enqueueOnce(registry.queues.extraction, 'extraction', ['post', postId, 'extraction', runId], payload).then(() => undefined),
}
...
const worker = runWorker(spec.queue, async (job) => {
  ...
  if (job.payload.kind === 'competitor' && !collectorAccountId) {
    const collector = await pool.query<{ id: string }>(`SELECT id FROM accounts WHERE role='collector' AND status='HEALTHY' AND (cooldown_until IS NULL OR cooldown_until <= now()) ORDER BY last_action_at ASC NULLS FIRST,id LIMIT 1`)
    collectorAccountId = collector.rows[0]?.id
  }
  return processJob({ ...job, payload: { ...job.payload, collectorAccountId, igUserId: String(job.payload.igUserId ?? igUserId) } as never })
})
...
process.once('SIGTERM', () => void stopHeartbeat().finally(() => pool.end()).finally(() => registry.connection.quit()))
