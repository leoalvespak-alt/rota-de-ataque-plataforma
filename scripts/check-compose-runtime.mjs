...
if (!workerEntrypoint.includes('exec node --import tsx --enable-source-maps /app/docker/worker-supervisor.mts "$engine"')) throw new Error('Worker entrypoint must exec one Node process per engine supervisor with the tsx loader')
const supervisorSource = readFileSync(path.join(root, 'docker', 'worker-supervisor.mts'), 'utf8')
if (!supervisorSource.includes('runWorker(worker as QueueName')) throw new Error('Supervisor must preserve a controlled paused runtime when an optional worker prerequisite blocks module initialization')
if (!supervisorSource.includes("state: 'worker_initialization_blocked'")) throw new Error('Supervisor prerequisite fallback must emit an explicit structured diagnostic')
