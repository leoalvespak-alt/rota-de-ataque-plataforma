...
try {
  ...
  if (direction === 'up') {
    ...
    const client = await pool.connect()
    try { await client.query('BEGIN'); await client.query(await readFile(path.join(migrationsDir, file), 'utf8')); await client.query('DELETE FROM schema_migrations WHERE version=$1', [latest]); await client.query('COMMIT') }
    catch (error) { await client.query('ROLLBACK'); throw error } finally { client.release() }
