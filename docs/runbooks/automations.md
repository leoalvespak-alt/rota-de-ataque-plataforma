...
O runtime reserva o heartbeat canônico antes da leitura assíncrona do estado desejado. Assim, workers que ainda mantêm instrumentação legada não podem sobrescrever `paused` com o estado antigo `disabled`; a saúde operacional deve reportar os 41 consumidores como `running` ou `paused`.

Um worker opcional sem credencial ou pré-requisito não derruba o supervisor nem desaparece do health: o boot registra `worker_initialization_blocked` com reason code e instala um consumidor controlado de fallback. Desligado, ele permanece `paused`; se uma execução for forçada depois de ligada, falha de modo rastreável como `PROVIDER_NOT_CONFIGURED` ou `PREREQUISITE_MISSING`. A configuração deve então ser corrigida e o supervisor reiniciado para carregar o processador real.
